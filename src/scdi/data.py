...
def build_panel(
    fact_demand: pd.DataFrame, calendar: pd.DataFrame, params: pd.DataFrame
) -> pd.DataFrame:
    df = fact_demand.merge(calendar, on="week_start", how="left").merge(
        params, on=["sku_id", "loc_id"], how="left"
    )
...
def add_lags(df: pd.DataFrame, lags=(1, 2, 3, 4, 8, 13, 26, 52)) -> pd.DataFrame:
    for lag in lags:
        df[f"lag_{lag}"] = df.groupby(["sku_id", "loc_id"])["sales_units"].shift(lag)
    return df
